...
<html>
    <head>
        ...
        <link rel="stylesheet" href="server/upload/resources/themes/bootstrap/font-awesome.min.css" /> <!-- 网站图标CSS式样 -->
    ...
    <body>
        ...
        <div class="path-announcement navbar navbar-default navbar-fixed-top">
            <div class="path-announcement2 container">
                <!-- 顶部公告栏 -->
		    <p><i class="fa fa-volume-down"></i>好好学习  天天向上</p>
            	<!-- 顶部公告栏 -->
            </div>
        </div>
		...
		<div class="page-content container"  id="container_page">
            ...
            <ul id="directory-listing" class="nav nav-pills nav-stacked">
                <?php foreach($dirArray as $name => $fileInfo): ?>
                    <li data-name="<?php echo $name; ?>" data-href="<?php echo "server/upload/" . $fileInfo['url_path']; ?>">
                        <a href="<?php echo "server/upload/" . $fileInfo['url_path']; ?>" class="clearfix" data-name="<?php echo $name; ?>">
                            <div class="row">
		...
		<div class="container readme-background" id="container_readme">
			<div class="readme">
				<center>
					<form action ="?user=admin" method ="post" id = "delform">
					<input type="hidden" name="pass" value = <?php echo "\"" . $lister->getPassword() . "\"" ?>/>
					<input type="hidden" name="del" id = "del" />
					<button onclick="delFunction()">删除</button>
					</form>
				</center>
			</div>
		</div>
